...
#if defined(NDEBUG)
# error "TigerStripe cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc6b61e28502da6a6111c9893a4d1551d3b192969a75c1edd63cbeffa31abe485");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TigerStripe: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TigerStripe Signed Message:\n";
...
{
    // TigerStripe: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // TigerStripe: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // TigerStripe: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // TigerStripe: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TigerStripe: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TigerStripe: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x029cfd947dafa38d72310acebc42c68f9d67b1ab233fea20e6ee7207dbc30e1d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Coindesk 23/Jul/2019 Bakkt begins testing Bitcoin futures";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a96adb274f97f16d9ed63fd1f00433508094e044ea7ab9bcaf37277ba99c693960340b2fb5667ae7debe4e59fd3c0dfb49cdc166102a0f8333821a66393aae12") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1563925472;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084850973;

        if (fTestNet)
        {
            block.nTime    = 1563925457;
            block.nNonce   = 386514980;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x607c5995eefb8b66e754cc71ddfcf33eca19bd7fa7a17805804a8ca83f75cbb8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb5, 0xda }; // TigerStripe: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TigerStripeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TigerStripeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TigerStripeMiner(CWallet *pwallet)
{
    printf("TigerStripeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tigerstripe-miner");
    ...
    try { loop {
        ...
        printf("Running TigerStripeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TigerStripeMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TigerStripeMiner, pwallet));
}
